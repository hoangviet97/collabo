...
export const createTag = ({ project_id, name, color }) => async (dispatch) => {
  try {
    const res = await axios.post(`http://localhost:9000/api/${project_id}/tags/add`, { name, color });
...
export const createTaskTag = ({ project_id, task, tag }) => async (dispatch) => {
  try {
    const res = await axios.post(`http://localhost:9000/api/${project_id}/tags/task-tag`, { task, tag });
...
export const getTags = ({ project_id }) => async (dispatch) => {
  try {
    ...
    const res = await axios.get(`http://localhost:9000/api/${project_id}/tags`);
...
export const getTagsByTasks = ({ project_id }) => async (dispatch) => {
  try {
    const res = await axios.get(`http://localhost:9000/api/${project_id}/tags/tasks`);
...
export const deleteTag = ({ project_id, id }) => async (dispatch) => {
  try {
    const res = await axios.delete(`http://localhost:9000/api/${project_id}/tags/${id}`);
